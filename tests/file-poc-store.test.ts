...
describe("FilePocStore", () => {
  it("persists PoC records, requirements, plans, and setup results across instances", async () => {
    const dir = mkdtempSync(join(tmpdir(), "poc-store-"));
    const path = join(dir, "pocs.json");

    try {
      const first = new FilePocStore(path);
      ...
      const second = new FilePocStore(path);
...
function monitoringReport(
  ...
  return {
    ...
    planVersion: 1,
    runId,
    checkedAt: runId === "monitor-run-1" ? "2026-06-05T00:00:00.000Z" : "2026-06-05T12:00:00.000Z",
    window: {
